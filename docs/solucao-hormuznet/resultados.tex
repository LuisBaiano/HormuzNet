\subsection{Ambiente de Testes}

A validação do HormuzNet foi conduzida utilizando o \textit{Docker Compose} para simular um ambiente distribuído em rede local. Foram instanciados 9 \textit{brokers}, 7 drones autônomos e 18 sensores (2 para cada setor). O monitoramento foi realizado através do dashboard tático acessível via navegador na porta 8085.

\subsection{Análise de Resiliência e Consistência}

A Tabela~\ref{tab:resiliencia} apresenta os resultados dos testes de estresse e falhas injetadas no sistema para verificar a capacidade de recuperação da malha.

\begin{table}[ht]
  \centering
  \caption{Testes de resiliência e comportamento da malha.}
  \label{tab:resiliencia}
  \begin{tabular}{p{1.5cm}p{4.5cm}p{5.5cm}c}
    \toprule
    \textbf{ID} & \textbf{Cenário de Falha} & \textbf{Resposta do Sistema} & \textbf{Status} \\
    \midrule
    TR-01 & Queda do Broker B3 (Nordeste) & B2 assume Setor Nordeste via Ring Failover; drones fazem \textit{fallback} & OK \\
    TR-02 & Sensor Multicast & Alerta recebido simultaneamente por todos os brokers & OK \\
    TR-03 & Drone Abatido & Broker detecta perda e reenvia alerta para a fila global & OK \\
    TR-04 & Conflito Lamport & Brokers ordenam ocorrências identicamente via timestamps & OK \\
    TR-05 & Partição de Rede & Brokers isolados operam regionalmente e sincronizam ao retornar & OK \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Desempenho da Sincronização}

As métricas de latência e processamento foram coletadas durante uma simulação de 10 minutos com alta taxa de ocorrências (1 evento a cada 2 segundos). Os resultados estão resumidos na Tabela~\ref{tab:performance}.

\begin{table}[ht]
  \centering
  \caption{Métricas de desempenho e sincronização.}
  \label{tab:performance}
  \begin{tabular}{p{5cm}p{3.5cm}p{4.5cm}}
    \toprule
    \textbf{Métrica} & \textbf{Valor Observado} & \textbf{Observação} \\
    \midrule
    Tempo de Despacho (médio) & $\approx$\,1.5 s & Da ocorrência ao envio do drone \\
    Divergência de Lamport & 0 ms & Filas mantiveram-se idênticas \\
    Uso de CPU por Broker & $<$ 5\% & Eficiência das goroutines \\
    Taxa de Sucesso de Missões & $\approx$\,91\% & Impacto do modelo de 10\% de falha \\
    Latência P2P entre Brokers & $<$ 10 ms & Comunicação TCP em rede local \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Discussão dos Resultados}

Os experimentos demonstraram que a descentralização atingiu o objetivo de eliminar SPOFs. A queda de um \textit{broker} (ex: B3) não interrompeu o monitoramento de sua região, pois o vizinho imediato (B2) assumiu a herança do setor órfão via Ring Failover em tempo real, enquanto o \textit{fallback} dos drones para nós vizinhos ocorreu sem perdas. A consistência garantida pelo algoritmo de Lamport e pela exclusão mútua distribuída (ownership da conexão TCP de cada drone) evitou que múltiplos drones atendessem a mesma ocorrência. Adicionalmente, a de-duplicação de eventos implementada no Monitor permitiu manter a interface tática limpa e livre de duplicatas de fofoca, mesmo sob alta carga de ocorrências concorrentes.

Além disso, a implementação de validação de identidade da conexão ativa nos blocos de limpeza resolveu o problema de esgotamento de drones disponíveis que surgia a longo prazo. Anteriormente, com a alta rotatividade gerada por abates, conexões obsoletas que se encerravam tardiamente desativavam a chave dos drones nos brokers locais, deixando-os inativos de forma permanente mesmo marcados como disponíveis. Com a correção das condições de corrida de limpeza de conexão, a malha demonstrou capacidade de rodar indefinidamente mantendo o despacho contínuo de tarefas sem acúmulo de fila pendente.

A principal limitação observada foi a escalabilidade do \textit{multicast} em redes que não suportam IGMP de forma eficiente, o que poderia restringir o sistema em ambientes de rede WAN complexos. Contudo, no escopo de uma rede de monitoramento regional, a solução mostrou-se extremamente robusta.
